...
class ResNet(Backbone):
    ...
    def forward(self, x, output_layers=None):
        ...
        if self._add_output_and_check('layer1', p1, outputs, output_layers):
            return outputs

        p2 = self.layer2(p1)

        if self._add_output_and_check('layer2', p2, outputs, output_layers):
            return outputs

        p3 = self.layer3(p2)

        if self._add_output_and_check('layer3', p3, outputs, output_layers):
            return outputs

        p4 = self.layer4(p3)

        if self._add_output_and_check('layer4', p4, outputs, output_layers):
            return outputs

        out = [p1, p2, p3]
        if self._add_output_and_check('last', out, outputs, output_layers):
            return outputs
...
def resnet50(output_layers=None, pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet50'], model_dir='/home/zhanghc@1/4_tracking_dataset/pretrain'))
    return model
