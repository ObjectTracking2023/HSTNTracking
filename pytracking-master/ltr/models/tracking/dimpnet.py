...
class DiMPnet(nn.Module):
    ...
    def __init__(self, feature_extractor, classifier, bb_regressor, classification_layer, bb_regressor_layer, Neck, Channelatt, Spatialatt, Memory):
        super().__init__()

        self.neck = Neck
        self.channelatt = Channelatt
        ...
        self.output_layers = sorted(list(set(self.classification_layer + self.bb_regressor_layer + ['layer1'])))


    def forward(self, train_imgs, test_imgs, mmr_imgs, train_bb, test_proposals, *args, **kwargs):
        ...
        mmr_feat = self.extract_backbone_features(mmr_imgs.reshape(-1, *train_imgs.shape[-3:]))
        test_feat = self.extract_backbone_features(test_imgs.reshape(-1, *test_imgs.shape[-3:]))
        train_feat2 = self.neck(train_feat)
        mmr_feat2 = self.neck(mmr_feat)
        train_feat2 = self.memory(train_feat2, mmr_feat2, train_feat2)
        train_feat2 = self.channelatt(torch.cat(train_feat2, 1))       
        train_feat2 = [train_feat2[:, 0:256, :, :], train_feat2[:, 256:512, :, :], train_feat2[:, 512:768, :, :]]      
        train_feat2 = self.spatialatt(train_feat2[0], train_feat2[1], train_feat2[2], train_feat['layer3'])

        # Classification features
        train_feat['layer3'] = train_feat2

        del train_feat['layer1']
        del test_feat['layer1']
...
def dimpnet50(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
    ...
    neck = AdjustAllLayer([256, 512, 1024], [256, 256, 256])
    channelatt = SEModule(256 * 3, 3)
    spatialatt = NONLocalBlock2D(in_channels=256, inter_channels=256)
    memory = CTFM()

    # Classifier features
    if classification_layer == 'last':
        feature_dim = 64
    elif classification_layer == 'layer3':
        feature_dim = 256
    ...
    net = DiMPnet(feature_extractor=backbone_net, classifier=classifier, bb_regressor=bb_regressor,
                  classification_layer=classification_layer, bb_regressor_layer=['layer2', 'layer3'],
                  Neck = neck, Channelatt = channelatt, Spatialatt = spatialatt, Memory = memory)
    return net
