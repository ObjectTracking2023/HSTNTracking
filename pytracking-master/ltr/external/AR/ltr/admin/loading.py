...
import importlib
import inspect
from ltr.external.AR.ltr.admin import settings as ws_settings
...
def _setup_legacy_env():
    ...
    for m in ('model_constructor', 'stats', 'settings', 'local'):
        importlib.import_module('ltr.external.AR.ltr.admin.' + m)
        sys.modules['dlframework.common.utils.' + m] = sys.modules['ltr.external.AR.ltr.admin.' + m]
