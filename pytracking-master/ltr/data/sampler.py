...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if is_video_dataset:
            ...
            if self.frame_sample_mode == 'interval':
                ...
                while test_frame_ids is None:
                    ...
                    base_frame_id_m = self._sample_visible_ids(visible, num_ids=1, min_id=self.num_train_frames - 1,
                                                               max_id=len(visible) - self.num_test_frames)
                    prev_frame_ids_m = self._sample_visible_ids(visible, num_ids=self.num_train_frames - 1,
                                                                min_id=base_frame_id[0] - self.max_gap - gap_increase,
                                                                max_id=base_frame_id[0])
                    if prev_frame_ids is None:
                        gap_increase += 5
                        continue
                    if prev_frame_ids_m is None:
                        gap_increase += 5
                        continue
                    train_frame_ids = base_frame_id + prev_frame_ids

                    memory_frame_ids = base_frame_id_m + prev_frame_ids_m
            ...
            memory_frame_ids = [1] * self.num_train_frames
            test_frame_ids = [1] * self.num_test_frames

        memory_frame_ids.sort()
        ...
        memory_frames, memory_anno, meta_obj_memory = dataset.get_frames(seq_id, memory_frame_ids, seq_info_dict)

        data = TensorDict({'train_images': train_frames,
                           'train_anno': train_anno['bbox'],
                           'test_images': test_frames,
                           'test_anno': test_anno['bbox'],
                           'mmr_images': memory_frames,
                           'mmr_anno': memory_anno['bbox'],
                           'dataset': dataset.get_name(),
                           'test_class': meta_obj_test.get('object_class_name')})
        a = self.processing(data)
        return a
