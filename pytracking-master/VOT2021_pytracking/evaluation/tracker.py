...
import torch
from ltr.external.AR.pytracking.ARcm_seg import ARcm_seg
import cv2
from ltr.external.vot.vot20_utils import *
...
class Tracker:
    ...
    def run_vot2020(self, debug=None, visdom_info=None):
        ...
        parser.add_argument('--AR_model',
                            default='/home/ori/pytracking-master/ltr/external/AR/ltr/checkpoint/ARcm_coco_seg/ARnet_seg_mask_ep0040.pth',
                            type=str,
        ...
        alpha_model = ARcm_seg(args.AR_model, input_sz=384)
        if debug is None:
            visualization_ = getattr(params, 'visualization', False)
        else:
            visualization_ = True if debug else False
        ...
        imagevot = cv2.cvtColor(cv2.imread(image_path), cv2.COLOR_BGR2RGB)
        H, W, _ = imagevot.shape
        mask = make_full_size(vot_anno, (imagevot.shape[1], imagevot.shape[0]))
        regionvot = rect_from_mask(mask)
        gt_bbox_np = np.array(regionvot).astype(np.float32)
        alpha_model.initialize(imagevot, np.array(gt_bbox_np))
        ...
        while True:
            ...
            h = y2 - y1
            bbox_new = [x1, y1, w, h]
            pred_mask, search, search_mask = alpha_model.get_mask(image, np.array(bbox_new), vis=True)
            final_mask = (pred_mask > 0.5).astype(np.uint8)

            prev_output = OrderedDict(out)

            if output_segmentation:
                handle.report(final_mask)
            else:
