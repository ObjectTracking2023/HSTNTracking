...
class Tracker:
    ...
    def create_tracker(self, params):
        tracker = self.tracker_class(params) # eg: class DIMP(params) now initialize
    ...
    def run_sequence(self, seq, visualization=None, debug=None, visdom_info=None, multiobj_mode=None):
        ...
        init_info = seq.init_info() # dict {'init_bbox': [x0,y0,w0,h0]} include 'init_bbox' 'init_mask'
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        # Just modify for vot series datasets
        if seq.dataset == 'vot':
            frame_counter = 0
            lost_number = 0
            for idx, (img, gt_bbox) in enumerate(zip(seq.frames, seq.ground_truth_rect)):
                '''
                '''
                if idx == frame_counter:
                    image = self._read_image(img)

                    gt_bbox_ = convert_vot_anno_to_rect(gt_bbox, type='preserve_area') # [x,y,w,h]
                    ...
                    out = tracker.initialize(image, {'init_bbox': gt_bbox_})  # out = {'time': ??}
                    out1 = {'time': out['time']} if out is not None else {}
                    if out is not None:
                        out = {'target_bbox': gt_bbox_}

                    prev_output = OrderedDict(out)
                    init_default = {'target_bbox': 1,
                                    'time': time.time() - start_time}

                    _store_outputs(out1, init_default)
                elif idx > frame_counter:
                    image = self._read_image(img)
                    start_time = time.time()

                    info = dict() # actually return a dict
                    info['previous_output'] = prev_output

                    out = tracker.track(image, info)  # it seems info has not used # out {'target_bbox': [x,y,w,h]}
                    prev_output = OrderedDict(out)
                    overlap = vot_overlap(out['target_bbox'], gt_bbox, (image.shape[1], image.shape[0]))
                    if overlap > 0:
                        # not lost
                        _store_outputs(out, {'time': time.time() - start_time})
                    else:
                        # lost object
                        _store_outputs({'target_bbox':2}, {'time': time.time() - start_time})
                        frame_counter = idx + 5 # skip 5 frames
                        lost_number += 1
                else:
                    _store_outputs({'target_bbox':0}, {'time': 1e-5})
            ...
            print('Lost: {:d}'.format(lost_number))
            ...
            start_time = time.time()
            out = tracker.initialize(image, init_info)  # out = {'time': ??}
            if out is None:
                out = {}
            ...
            init_default = {'target_bbox': init_info.get('init_bbox'),
                            'time': time.time() - start_time,
                            'segmentation': init_info.get('init_mask')}  # None
            ...
            for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
                ...
                out = tracker.track(image, info) # it seems info has not used
                prev_output = OrderedDict(out)
                _store_outputs(out, {'time': time.time() - start_time})
