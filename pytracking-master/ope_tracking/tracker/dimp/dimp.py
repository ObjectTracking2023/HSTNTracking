...
from ltr.models.stdomain.condition import updated_condition
    ...
    def forward(self, features):
        if self.num == 1:
            ...
            for i in range(self.num):
                ...
                out.append(adj_layer(features['layer{}'.format(i + 1)]))
            return out
...
class DiMP(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        self.net.net=self.net.net.eval()
        ...
        self.backbone_feat = init_backbone_feat
    ...
    def track(self, image, info: dict = None) -> dict:
        global updated_flag
        updated_flag = False
        #memory = CTFM().cuda().eval()

        self.debug_info = {}

        self.frame_num += 1
        self.debug_info['frame_num'] = self.frame_num
        uc = updated_condition()

        if self.frame_num == 3:
            updated_flag = True
        else:
            if (self.frame_num - self.updated_frame < 20):
                updated_flag = False
            else:
                updated_flag = uc(self.bs)
        ...
        self.sample_pos, self.sample_scales = sample_pos, sample_scales
        #neck = AdjustAllLayer([256, 512, 1024], [256, 256, 256]).cuda().eval()
        #channelatt = SEModule(256 * 3, 3).cuda().eval()
        #spatialatt = NONLocalBlock2D(in_channels=256, inter_channels=256).cuda().eval()

        y = self.net.neck(backbone_feat)

        if updated_flag == True:
            with torch.no_grad():
                memory_filer = self.net.memory(self.cla, self.memory_feature, self.cla)
                train_feat2 = self.net.channelatt(torch.cat(memory_filer, 1))
                train_feat2 = [train_feat2[:, 0:256, :, :], train_feat2[:, 256:512, :, :], train_feat2[:, 512:768, :, :]]
                train_feat2 = self.net.spatialatt(train_feat2[0], train_feat2[1], train_feat2[2], backbone_feat['layer3'])
            backbone_feat['layer3'] = train_feat2
            if (len(backbone_feat) == 3):
                del backbone_feat['layer1']
            memory_x = self.get_classification_features(backbone_feat)
            train_x = memory_x[self.scale_ind:self.scale_ind + 1, ...]

            # Create target_box and label for spatial sample
            target_box = self.get_iounet_box(self.pos, self.target_sz, self.sample_pos[self.scale_ind, :],
                                             self.sample_scales[self.scale_ind])

            # Update the classifier model
            self.update_classifier(train_x, target_box, self.learning_rate, self.s[self.scale_ind, ...])
            self.memory_feature = y
            self.updated_frame = self.frame_num
        if self.frame_num == 2:
            self.memory_feature = y
            self.updated_frame = 2
            self.cla = self.memory_feature
        ...
        self.s = s
        self.translation_vec, self.scale_ind, self.s, self.flag = translation_vec, scale_ind, s, flag
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        # spatialatt = NONLocalBlock2D(in_channels=256, inter_channels=256).cuda().eval()
        with torch.no_grad():
            if isinstance(init_backbone_feat, list):
                y = init_backbone_feat
                self.cla = y
                y = torch.cat(y, 1)
                y = self.net.channelatt(y)
                y = [y[:, 0:256, :, :], y[:, 256:512, :, :], y[:, 512:768, :, :]]
                y = self.net.spatialatt(y[0], y[1], y[2], self.backbone_feat['layer3'])
                self.backbone_feat['layer3'] = y
                if len(self.backbone_feat) == 3:
                    del self.backbone_feat['layer1']
                x = self.get_classification_features(self.backbone_feat)
            else:
                y = self.net.neck(init_backbone_feat)
                self.cla = y
                y = torch.cat(y, 1)
                y = self.net.channelatt(y)
                y = [y[:, 0:256, :, :], y[:, 256:512, :, :], y[:, 512:768, :, :]]
                y = self.net.spatialatt(y[0], y[1], y[2], init_backbone_feat['layer3'])
                init_backbone_feat['layer3'] = y
                if len(init_backbone_feat) == 3:
                    del init_backbone_feat['layer1']
